...
?>
							...
							<h2 style="color: green">UNIDAD EDUCATIVA PARTICULAR "Young Living"</h2>
								...
								<tr>
								<td> <h6 class="border">Registro de Asistencia </h6></td>
							...
							<div id="rst"></div>
					<table class="table  table-bordered  table-sm" style=" width:50%; margin-left: 20%;">
						<tr>
							<th ><p style="text-align: center;">Estudiantes</p> </th>
							<th width="50"> <p > FJ </p> </th>
							<th width="50"> <p > FI </p> </th>
							<th width="50"> <p > AT </p> </th>
						...
						<?php 
						$sql="SELECT a.nombre_Alumno , ca.fk_alumno from alumno a , curso_alumno ca where ca.fk_alumno=a.id_alumno and ca.fk_curso='$curso' ";
						$result=mysqli_query($conexion,$sql);
						while($mostrar=mysqli_fetch_row($result)){
	?>
						<tr>
 				<td> <input type="text" name="nombre"  class="form-control-plaintext" value="<?php echo $mostrar[0];?>" readonly > </td>
  				<td >  <input class="form-control" type="text" name="fj[]" id="fj[]" value=""  onKeypress="if (event.keyCode < 45 || event.keyCode > 57) event.returnValue = false;">	 </td>
  				<td >  <input class="form-control" type="text" name="fi[]" id="fi[]" value="" onKeypress="if (event.keyCode < 45 || event.keyCode > 57) event.returnValue = false;">	 </td>
  				<td >  <input class="form-control" type="text" name="at[]" id="at[]" value="" onKeypress="if (event.keyCode < 45 || event.keyCode > 57) event.returnValue = false;">	 </td>
...
<input type="submit" id="btnguardar" name="btnguardar" class="btn btn-primary" value="Guardar">
	...
	<script type="text/javascript">
		jQuery(document).on('submit','#frmnotas',function(event){
			...
			jQuery.ajax({
				url:"../procesos/Calificaciones/registroasistencia.php",
				type: 'POST',
				dataType:'json',
				data:$(this).serialize(),
			})
